...
extension WatchConnectivityManager: WCSessionDelegate {
    nonisolated func session(_ session: WCSession, activationDidCompleteWith activationState: WCSessionActivationState, error: Error?) {
        Task { @MainActor in
            if let error = error {
                ...
                // 앱 시작 시 현재 즐겨찾기 상태를 iOS 앱에 전송
                syncFavoriteIdsToPhone()
            }
        }
    }
    ...
    private func handleReceivedMessage(_ message: [String : Any]) {
        ...
        // iOS 앱에서 받은 즐겨찾기 데이터를 워치에 반영
        saveFavoriteIds(favoriteIds)
        print("✅ Received favoriteIds from phone: \(favoriteIds.count) items")
    ...
    @MainActor
    private func syncFavoriteIdsToPhone() {
        // iOS 앱과 동일한 키 사용
    ...
    @MainActor
    private func saveFavoriteIds(_ favoriteIds: Set<String>) {
        do {
            let data = try JSONEncoder().encode(favoriteIds)
            // iOS 앱과 동일한 키 사용
